...
func (a *Actions) createRoom(c *ws.Context) {
	...
	var (
		r   interface{ Snapshot() map[string]any }
		err error
	)
	var createdRoomID string
	if req.Game == "snake" {
		roomValue, createErr := a.service.Create("snake", 1, 8)
		if createErr != nil {
			err = createErr
		} else {
			r = roomValue
			createdRoomID = roomValue.ID
		}
	} else {
		if req.Players == 0 {
			req.Players = 2
		}
		roomValue, createErr := a.service.Create(req.Game, req.Players)
		if createErr != nil {
			err = createErr
		} else {
			r = roomValue
	...
	if err := a.service.Join(createdRoomID, playerID, displayName(req.Name, playerID)); err != nil {
		c.SendCode(400, err.Error())
		return
	}

	topic := roomTopic(createdRoomID)
	c.Sub(topic)
	state := r.Snapshot()
	c.Send(state)
	c.Pub(topic, state)
...
func (a *Actions) addBot(c *ws.Context) {
	playerID, ok := currentPlayer(c)
	if !ok { c.SendCode(401, "guest login required"); return }
	var req roomRequest
	if err := c.BindingJson(&req); err != nil || strings.TrimSpace(req.RoomID) == "" { c.SendCode(400, "invalid room request"); return }
	req.RoomID = strings.ToUpper(strings.TrimSpace(req.RoomID))
	if err := a.service.AddBot(req.RoomID, playerID); err != nil { c.SendCode(400, err.Error()); return }
...
func (a *Actions) rematch(c *ws.Context) {
	...
	if err := c.BindingJson(&req); err != nil || strings.TrimSpace(req.RoomID) == "" { c.SendCode(400, "invalid room request"); return }
	req.RoomID = strings.ToUpper(strings.TrimSpace(req.RoomID))
	if err := a.service.Rematch(req.RoomID, playerID); err != nil { c.SendCode(400, err.Error()); return }
	a.sendRoom(c, req.RoomID)
}
...
func (a *Actions) move(c *ws.Context) {
	...
	if !ok { c.SendCode(401, "guest login required"); return }
	var req moveRequest
	if err := c.BindingJson(&req); err != nil || strings.TrimSpace(req.RoomID) == "" || len(req.Move) == 0 { c.SendCode(400, "invalid move"); return }
	req.RoomID = strings.ToUpper(strings.TrimSpace(req.RoomID))
	if err := a.service.Move(req.RoomID, playerID, req.Move); err != nil { c.SendCode(400, err.Error()); return }
	a.sendRoom(c, req.RoomID)
}
...
func (a *Actions) tetrisState(c *ws.Context) {
	...
	if err := c.BindingJson(&req); err != nil || !a.canRelayTetris(req.RoomID, playerID) { c.SendCode(400, "invalid tetris state"); return }
...
func (a *Actions) tetrisAttack(c *ws.Context) {
	...
	if err := c.BindingJson(&req); err != nil || req.Lines <= 0 || req.Lines > 8 || !a.canRelayTetris(req.RoomID, playerID) { c.SendCode(400, "invalid tetris attack"); return }
	roomID := strings.ToUpper(strings.TrimSpace(req.RoomID))
...
func (a *Actions) tetrisGameOver(c *ws.Context) {
	...
	if !ok { c.SendCode(401, "guest login required"); return }
	var req roomRequest
	if err := c.BindingJson(&req); err != nil || !a.canRelayTetris(req.RoomID, playerID) { c.SendCode(400, "invalid tetris game over"); return }
	roomID := strings.ToUpper(strings.TrimSpace(req.RoomID))
...
func (a *Actions) tetrisRestart(c *ws.Context) {
	...
	if !ok { c.SendCode(401, "guest login required"); return }
	var req roomRequest
	if err := c.BindingJson(&req); err != nil || !a.canRelayTetris(req.RoomID, playerID) { c.SendCode(400, "invalid tetris restart"); return }
	roomID := strings.ToUpper(strings.TrimSpace(req.RoomID))
...
func (a *Actions) snakeStart(c *ws.Context) {
	...
	if !ok { c.SendCode(401, "guest login required"); return }
	var req roomRequest
	if err := c.BindingJson(&req); err != nil || strings.TrimSpace(req.RoomID) == "" { c.SendCode(400, "invalid snake start"); return }
	req.RoomID = strings.ToUpper(strings.TrimSpace(req.RoomID))
...
func (a *Actions) snakeInput(c *ws.Context) {
	...
	if err := c.BindingJson(&req); err != nil || strings.TrimSpace(req.RoomID) == "" { c.SendCode(400, "invalid snake input"); return }
	req.RoomID = strings.ToUpper(strings.TrimSpace(req.RoomID))
...
func (a *Actions) snakeRestart(c *ws.Context) {
	...
	if !ok { c.SendCode(401, "guest login required"); return }
	var req roomRequest
	if err := c.BindingJson(&req); err != nil || strings.TrimSpace(req.RoomID) == "" { c.SendCode(400, "invalid snake restart"); return }
	req.RoomID = strings.ToUpper(strings.TrimSpace(req.RoomID))
	pubsub := c.Client.Hub.PubSub
...
func (a *Actions) sendRoom(c *ws.Context, roomID string) {
	r, ok := a.service.Get(roomID)
	if !ok { c.SendCode(404, "room not found"); return }
	state := r.Snapshot()
	c.Send(state)
...
func currentPlayer(c *ws.Context) (game.PlayerID, bool) {
	if c == nil || c.Client == nil { return "", false }
	user, _, loggedIn := c.Client.LoginState()
	if !loggedIn || user == nil { return "", false }
	uid := user.Suid
	if !strings.HasPrefix(uid, "arcade:") { return "", false }

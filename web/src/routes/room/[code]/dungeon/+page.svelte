...
<main class="page">
  ...
  <section class="stage-shell">
    ...
    {#if ready && !panelOpen}
      <button class="hud-weapon" onclick={()=>setPanel(true)} aria-label={t('details')} title={t('details')}>
        <span class="hud-weapon-icon"><img src={HUD_WEAPON_ICON_URL} alt="" /></span>
        <span class="hud-weapon-copy"><strong>{stats.weapon ? `${rarityName(stats.weaponRarity)} · ${weaponModel.name ?? t('dungeonBlade')}` : t('none')}</strong><small>{stats.weapon ? `+${weaponModel.damage ?? 0} ${t('damage')}` : t('emptyWeapon')}</small></span>
      </button>
      ...
      <button class="potion-action" class:available={(stats.healthPotions ?? 0) > 0} onclick={usePotion} aria-label={t('usePotion')} title={t('usePotion')}>
...
<style>
  ...
  .page{height:100dvh;box-sizing:border-box;padding:0;display:grid;grid-template-rows:minmax(0,1fr);gap:0;background:#080a0d}
  header,footer{display:none}
  header,footer{width:min(1180px,100%);margin:0 auto;align-items:center;justify-content:space-between;gap:12px}
  ...
  .stage-shell{position:relative;justify-self:center;width:100%;height:100%;min-height:0;overflow:hidden;background:#050608;touch-action:none;user-select:none}.stage{position:absolute;inset:0;display:flex;align-items:center;justify-content:center}.stage :global(canvas){display:block!important;width:100%!important;height:100%!important;max-width:none!important;max-height:none!important;margin:auto!important}
  .hud-weapon,.hud-progress{position:absolute;top:12px;box-sizing:border-box;border:1px solid rgba(71,80,91,.42);background:rgba(15,18,23,.86);color:#f4f0e8;pointer-events:auto}.hud-weapon{left:max(12px,env(safe-area-inset-left));display:flex;align-items:center;width:176px;height:48px;padding:7px 8px;gap:8px;text-align:left;cursor:pointer}.hud-weapon-icon{display:grid;place-items:center;width:32px;height:32px;flex:0 0 auto;background:rgba(23,28,34,.72)}.hud-weapon-icon img{width:28px;height:28px;image-rendering:pixelated}.hud-weapon-copy{min-width:0;display:grid;gap:4px}.hud-weapon-copy strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font:800 10px ui-monospace,SFMono-Regular,Menlo,monospace}.hud-weapon-copy small{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#7d8792;font:800 8px ui-monospace,SFMono-Regular,Menlo,monospace}.hud-progress{right:max(12px,env(safe-area-inset-right));display:grid;width:116px;height:48px;padding:7px 9px;gap:2px}.hud-progress small{color:#7d8792;font:800 8px ui-monospace,SFMono-Regular,Menlo,monospace}.hud-progress strong{font:800 12px ui-monospace,monospace}.hud-progress em{position:absolute;right:8px;top:7px;color:#ff746c;font:800 7px ui-monospace,monospace;font-style:normal}.potion-action{position:absolute;top:68px;right:max(12px,env(safe-area-inset-right));z-index:40;display:flex;align-items:center;justify-content:center;gap:7px;width:58px;height:42px;padding:0;border:1px solid rgba(255,135,145,.42);border-radius:4px;background:rgba(15,18,23,.82);color:#9aa4ae;font:800 10px ui-monospace,SFMono-Regular,Menlo,monospace;cursor:pointer;opacity:.72;touch-action:manipulation}.potion-action.available{color:#ffb9bf;border-color:rgba(255,135,145,.78);opacity:1}.potion-action:active{transform:translateY(1px)}.potion-action .potion-bottle{transform:scale(.78)}.potion-bottle{position:relative;display:inline-block;width:15px;height:18px;flex:0 0 auto;border-radius:3px 3px 6px 6px;background:#b92f43;box-shadow:inset 0 -5px 0 rgba(82,12,25,.42),0 2px 7px rgba(0,0,0,.36)}.potion-bottle::before{content:"";position:absolute;left:4px;top:-5px;width:7px;height:6px;border-radius:2px 2px 1px 1px;background:#c9bea5}.potion-bottle::after{content:"";position:absolute;left:3px;top:4px;width:3px;height:6px;border-radius:2px;background:rgba(255,210,214,.52)}
  ...
  .touch-controls{display:none;position:absolute;inset:0;z-index:35;pointer-events:none;box-sizing:border-box}.joystick{position:absolute;left:16px;bottom:34px;pointer-events:auto}.touch-actions{position:absolute;right:16px;bottom:34px;display:flex;gap:12px;align-items:flex-end;max-width:calc(100% - 120px);pointer-events:auto}.touch-actions button{width:68px;height:68px;flex:0 0 auto;border-radius:50%;border:1px solid rgba(193,255,86,.55);background:rgba(15,18,23,.7);color:#c1ff56;font:900 10px ui-monospace,monospace}.touch-actions .skill{width:80px;height:80px;border-color:rgba(201,132,255,.65);color:#d7c4ff}
  ...
  @media(max-width:720px){.page{padding:0;grid-template-rows:minmax(0,1fr)}header strong{display:none}footer{display:none}.players span:first-child{display:none}.stage-shell{width:100%;height:100%}.stage :global(canvas){width:100%!important;height:100%!important;max-width:none!important;max-height:none!important}.hud-weapon{left:max(10px,env(safe-area-inset-left));width:176px}.hud-progress{right:max(10px,env(safe-area-inset-right));width:116px}.potion-action{top:68px;right:max(10px,env(safe-area-inset-right));width:56px;height:40px}.touch-controls{display:block}.joystick{left:max(10px,env(safe-area-inset-left));bottom:max(16px,env(safe-area-inset-bottom))}.joystick :global(.virtual-joystick){width:88px!important;height:88px!important}.joystick :global(.joystick-knob){left:29px!important;top:29px!important;width:30px!important;height:30px!important}.touch-actions{right:max(10px,env(safe-area-inset-right));bottom:max(16px,env(safe-area-inset-bottom));gap:6px}.touch-actions button{width:52px;height:52px}.touch-actions .skill{width:58px;height:58px}.stats-overlay{padding:8px;align-items:end}.stats-panel{width:100%;padding:16px}.stat-grid{grid-template-columns:1fr 1fr}.stat-grid>div{border-bottom:1px solid rgba(71,80,91,.4)}.panel-actions{grid-template-columns:1fr}}
  @media(orientation:landscape) and (max-height:600px){.page{padding:0;grid-template-rows:minmax(0,1fr);gap:0}header{display:none}footer{display:none}.stage-shell{width:100%;height:100%}.stage :global(canvas){width:100%!important;height:100%!important;max-width:none!important;max-height:none!important}.hud-weapon{left:max(8px,env(safe-area-inset-left));width:154px;height:42px;padding:5px 6px}.hud-weapon-icon{width:28px;height:28px}.hud-weapon-icon img{width:24px;height:24px}.hud-weapon-copy strong{font-size:9px}.hud-progress{right:max(8px,env(safe-area-inset-right));width:104px;height:42px;padding:5px 7px}.hud-progress strong{font-size:11px}.potion-action{top:60px;right:max(8px,env(safe-area-inset-right));width:52px;height:36px}.touch-controls{display:block}.joystick{left:max(8px,env(safe-area-inset-left));bottom:max(8px,env(safe-area-inset-bottom))}.joystick :global(.virtual-joystick){width:78px!important;height:78px!important}.joystick :global(.joystick-knob){left:26px!important;top:26px!important;width:26px!important;height:26px!important}.touch-actions{right:max(8px,env(safe-area-inset-right));bottom:max(8px,env(safe-area-inset-bottom));gap:5px}.touch-actions button{width:46px;height:46px;font-size:9px}.touch-actions .skill{width:52px;height:52px}}
</style>

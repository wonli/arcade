  ...
  <section class="stage-shell">
    ...
    <div class="game-hud">
      ...
      <div class="hud-actions">
        <button class="potion-chip" disabled={(stats.healthPotions??0)<=0 || stats.hp>=stats.maxHp} on:click={usePotion} aria-label={t('usePotion')}>
    ...
    {#if panelOpen}
      <div class="stats-overlay" role="dialog" aria-modal="true" aria-label={t('details')}>
        <div class="stats-panel">
          ...
          <div class="panel-actions"><button class="use-potion" disabled={(stats.healthPotions??0)<=0 || stats.hp>=stats.maxHp} on:click={usePotion}><span class="potion-bottle small" aria-hidden="true"></span>{t('usePotion')} · {stats.healthPotions??0}</button><button class="resume" on:click={()=>setPanel(false)}>{t('close')}</button></div>
        </div>
      </div>
    {/if}
...
<style>
  ...
  .touch-controls{display:none;position:absolute;inset:0;z-index:45;pointer-events:none}.joystick-slot{position:absolute;left:max(16px,env(safe-area-inset-left));bottom:max(14px,env(safe-area-inset-bottom));pointer-events:auto}.touch-actions{position:absolute;right:max(16px,env(safe-area-inset-right));bottom:max(18px,env(safe-area-inset-bottom));display:flex;align-items:flex-end;gap:12px;pointer-events:auto}.touch-button{width:72px;height:72px;border-radius:50%;border:1px solid rgba(244,240,232,.38);background:rgba(15,18,23,.68);color:#f4f0e8;font:800 10px ui-monospace,monospace;touch-action:manipulation;box-shadow:0 5px 15px rgba(0,0,0,.28)}.touch-button.skill{width:84px;height:84px;color:#d7c4ff;border-color:rgba(201,132,255,.64);background:rgba(74,38,96,.62)}.touch-button.interact{color:#c1ff56;border-color:rgba(193,255,86,.55)}

<script>
  ...
  let error = ''
  let locale = 'en'
  let stats = { hp: 100, maxHp: 100, damage: 10, kills: 0, floor: 1, weapon: null, weaponRarity: null }
  let eventText = ''
...
<main class="page">
  ...
  <section class="hud">
    ...
    <div><span>{t('weapon')}</span><strong class:common={stats.weaponRarity === 'common'} class:uncommon={stats.weaponRarity === 'uncommon'} class:rare={stats.weaponRarity === 'rare'} class:epic={stats.weaponRarity === 'epic'}>{stats.weapon ? `${rarityName(stats.weaponRarity)} ${t('dungeonBlade')}` : t('none')}</strong></div>
  </section>

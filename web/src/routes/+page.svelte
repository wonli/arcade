<script>
  ...
  function description() {
    ...
    return '1–8 players. One arena. Host starts.'
  }
</script>
...
<main class="home">
  <section class="home-card">
    ...
    <div class="game-picker" aria-label="Choose a game">
      <button class:active={game === 'gomoku'} onclick={() => selectGame('gomoku')}>
        <span>01</span><strong>Gomoku</strong><small>Five in a row</small>
      </button>
      <button class:active={game === 'tetris'} onclick={() => selectGame('tetris')}>
        <span>02</span><strong>Tetris Battle</strong><small>Clear lines. Send garbage.</small>
      </button>
      <button class:active={game === 'snake'} onclick={() => selectGame('snake')}>
        <span>03</span><strong>Snake Arena</strong><small>1–8 players · Host starts</small>
      </button>
    </div>
    ...
    <button class="create" onclick={createRoom}>
      {game === 'snake' ? 'Create Snake Arena' : game === 'gomoku' ? 'Start 2 player Gomoku' : `Start ${players === 1 ? 'solo' : '2 player'} Tetris`}
    </button>

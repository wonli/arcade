    ...
    <div class="arena-layout">
      <div class="play-zone">
        ...
        {#if state?.status !== 'finished'}
          <div class="touch-pad" aria-label={t('snake.touchHint')}>
            <button class="up" aria-label={t('snake.up')} onpointerdown={(event)=>{event.preventDefault();sendDirection('up')}}>↑</button>
            <button class="left" aria-label={t('snake.left')} onpointerdown={(event)=>{event.preventDefault();sendDirection('left')}}>←</button>
            <button class="down" aria-label={t('snake.down')} onpointerdown={(event)=>{event.preventDefault();sendDirection('down')}}>↓</button>
            <button class="right" aria-label={t('snake.right')} onpointerdown={(event)=>{event.preventDefault();sendDirection('right')}}>→</button>
            <small>{t('snake.touchHint')}</small>
          </div>
        {/if}
      </div>
      <aside class="leaderboard">
        <span>{t('common.scoreboard')}</span>
        {#each state?.snakes??[] as snake,index}<div class:dead={!snake.alive} class="score-row"><i data-player={index}></i><div class="player-info"><strong>{snake.name}</strong><small>{snake.alive?t('snake.alive'):t('snake.out')}</small></div><div class="score-value"><small>{t('common.score')}</small><b>{snake.score}</b></div></div>{/each}
        <div class="keys">{t('snake.keyboard')}</div>
      </aside>
    </div>

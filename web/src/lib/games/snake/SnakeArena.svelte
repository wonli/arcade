      ...
      <aside class="leaderboard">
        ...
        {#each state?.snakes ?? [] as snake, index}
          <div class:dead={!snake.alive} class="score-row">
            ...
            <div class="player-info"><strong>{snake.name}</strong><small>{snake.alive ? 'ALIVE' : 'OUT'}</small></div>
            <div class="score-value"><small>SCORE</small><b>{snake.score}</b></div>
          </div>
        {/each}

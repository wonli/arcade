<script>
  ...
  function direction(event, value){ event.preventDefault(); onDirection?.(value) }
</script>
...
<div class:compact class="arena-layout">
  <div class="play-zone">
    ...
    {#if showTouchControls}
      <div class="touch-pad" aria-label={touchHint}>
        <button class="up" aria-label={upLabel} onpointerdown={(event)=>direction(event,'up')}>↑</button>
        <button class="left" aria-label={leftLabel} onpointerdown={(event)=>direction(event,'left')}>←</button>
        <button class="down" aria-label={downLabel} onpointerdown={(event)=>direction(event,'down')}>↓</button>
        <button class="right" aria-label={rightLabel} onpointerdown={(event)=>direction(event,'right')}>→</button>
        <small>{touchHint}</small>
      </div>
    {/if}
  </div>
...
<style>
  ...
  @media(hover:none),(pointer:coarse){.arena-layout:not(.compact) .touch-pad{display:grid}.arena-layout:not(.compact) .keys{display:none}}
  @media(max-width:820px){.arena-layout:not(.compact){grid-template-columns:1fr;gap:18px}.arena-layout:not(.compact) .leaderboard{display:grid;grid-template-columns:repeat(2,1fr);gap:0 16px}.arena-layout:not(.compact) .leaderboard>span,.arena-layout:not(.compact) .keys{grid-column:1/-1}.arena-layout:not(.compact) .snake-board{width:100%;box-shadow:6px 6px 0 #050607}.arena-layout:not(.compact) .touch-pad{display:grid}}
  @media(max-width:520px){.arena-layout:not(.compact) .leaderboard{grid-template-columns:1fr}.arena-layout:not(.compact) .touch-pad{grid-template-columns:repeat(3,54px);grid-template-rows:repeat(2,48px) auto}}
</style>

      ...
      for (let y = wall.y / g.grid.tileSize; y < (wall.y + wall.height) / g.grid.tileSize; y++) {
        for (let x = wall.x / g.grid.tileSize; x < (wall.x + wall.width) / g.grid.tileSize; x++) {
          const inOpening = wall.orientation === 'vertical'
            ? wall.passage && y * g.grid.tileSize >= wall.passage.y && y * g.grid.tileSize < wall.passage.y + wall.passage.height
            : wall.passage && x * g.grid.tileSize >= wall.passage.x && x * g.grid.tileSize < wall.passage.x + wall.passage.width

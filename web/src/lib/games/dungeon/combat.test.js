...
test('picking up a health potion stores it without changing hp', () => {
  const low = applyPickup({ hp: 40, maxHp: 100, healthPotions: 0 }, { type: 'consumable.health_potion', heal: 28 })
  const high = applyPickup({ hp: 90, maxHp: 100, healthPotions: 2 }, { type: 'consumable.health_potion', heal: 28 })
  assert.equal(low.hp, 40); assert.equal(low.healthPotions, 1)
  assert.equal(high.hp, 90); assert.equal(high.healthPotions, 3)
})

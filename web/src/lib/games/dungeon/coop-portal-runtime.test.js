...
test('team countdown starts only when both living players are inside and resets when either leaves', () => {
  const { scene, p1, p2, facts } = fixture()
  ...
  assert.equal(p1.portalCountdownLabel, null)
  assert.equal(p2.portalCountdownLabel, null)
...
test('follower never advances locally and only renders authority countdown facts', () => {
  const { scene, p1, p2, runtime, advances } = fixture({ authority: false })
  p2.state.x = 104
  scene.updatePortal(1000)
  scene.updatePortal(5000)
  ...
  runtime.applyFact({ type: 'portal.dwell', entityId: 'portal:R:1:0', active: true, seconds: 2, playerIds: ['p1', 'p2'] })
  assert.equal(p1.portalCountdownLabel?.text, '2')
  assert.equal(p2.portalCountdownLabel?.text, '2')
  runtime.applyFact({ type: 'portal.dwell', entityId: 'portal:R:1:0', active: false, seconds: null, playerIds: [] })
  assert.equal(p1.portalCountdownLabel, null)
  assert.equal(p2.portalCountdownLabel, null)
})

const BUILDS = [
  {
    ...
    label: 'Lightning',
    wants: ['thunder', 'chain'],
    affinity: { dagger: 1, sword: 1.08, katana: 0.96 },
  },
  {
    id: 'reaper',
    label: 'Reaper',
    wants: ['executioner', 'corpse_burst'],
    affinity: { dagger: 0.92, sword: 1, katana: 1.12 },
  },
  {
    id: 'blood-rush',
    label: 'Blood Rush',
    wants: ['life_steal', 'critical_heal', 'attack_speed'],
    affinity: { dagger: 1.16, sword: 1, katana: 0.88 },
  },
  {
    id: 'tempest',
    label: 'Tempest',
    wants: ['whirlwind', 'skill_radius', 'skill_haste'],
  ...
  },
  {
    ...
    affinity: { dagger: 1.04, sword: 1, katana: 1.12 },
  },
]
...
export function classifyBuild(item = {}) {
  ...
  if (!best?.matches) {
    return { id: 'open', label: 'Open Build', matches: 0, score: 0 }
  }
...
export function buildHint(item) {
  ...
  if (build.id === 'open') return ''

  const target = BUILDS.find((entry) => entry.id === build.id)
  return `${build.label} ${build.matches}/${target.wants.length}`
}

<script>
  ...
  function resultTitle() {
    ...
    return state.winner === myColor ? 'You win' : 'You lose'
  }
...
<div class="chess-wrap" onpointerdown={unlockAudio}>
  ...
  {#if state?.status === 'playing' && myColor}
    <div class="chess-actions">
      <button class="resign-button" onclick={resign} disabled={actionBusy}>Resign</button>
      {#if actionError}<span>{actionError}</span>{/if}
    </div>
  {/if}
  ...
  {#if state?.status === 'finished'}
    <div class="chess-result-modal" role="dialog" aria-modal="true" aria-label="Chess result">
      <div class="result-card">
        <span class="result-kicker">GAME OVER</span>
        <h2>{resultTitle()}</h2>
        <p>{resultDetail()}</p>
        {#if actionError}<div class="result-error">{actionError}</div>{/if}
        <div class="result-actions">
          {#if myColor}<button class="play-again" onclick={rematch} disabled={actionBusy}>Play again</button>{/if}

...
func (s *Service) StartSnake(roomID string, playerID game.PlayerID, publish SnakePublisher) error {
	...
	if r.GameName != "snake" {
		return errors.New("room is not snake")
	}
	if r.HostID != playerID {
		return errors.New("only host can start snake")
	}
	if r.Status != room.StatusWaiting {
		return errors.New("snake room is not waiting")
	}
	players := r.Players
	if len(players) < r.MinPlayers || len(players) > r.MaxPlayers {
		return errors.New("invalid snake player count")
	}
...
func (s *Service) SnakeInput(roomID string, playerID game.PlayerID, direction snake.Direction) error {
	...
	s.snakeMu.Unlock()
	if runtime == nil {
		return errors.New("snake game is not running")
	}
...
func (s *Service) RestartSnake(roomID string, playerID game.PlayerID, publish SnakePublisher) error {
	...
	if r.HostID != playerID {
		return errors.New("only host can restart snake")
	}
	if r.Status != room.StatusFinished {
		return errors.New("snake game is not finished")
	}
...
func (s *Service) startSnakeRuntime(r *room.Room, players []room.Player, publish SnakePublisher) error {
	s.snakeMu.Lock()
	if existing := s.snakes[r.ID]; existing != nil {
		close(existing.stop)
		delete(s.snakes, r.ID)
	}
	enginePlayers := make([]snake.Player, 0, len(players))
	for _, player := range players {
		enginePlayers = append(enginePlayers, snake.Player{ID: player.ID, Name: player.Name})
	}
	runtime := &snakeRuntime{
		game: snake.New(enginePlayers, func(n int) int {
			if n <= 1 {
				return 0
			}
			return rand.IntN(n)

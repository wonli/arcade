...
// builds replace the directory contents with SvelteKit output before Go compiles.
//go:embed all:dist
...
func registerFS(engine *gin.Engine, root fs.FS) {
	...
	engine.NoRoute(func(c *gin.Context) {
		...
		// Generated bundles must fail loudly instead of being replaced by the
		// app shell. SvelteKit emits its client assets under _app/; assets/ is
		// kept here as well so a future asset layout cannot be silently masked.
		if strings.HasPrefix(name, "_app/") || strings.HasPrefix(name, "assets/") {
			c.Status(http.StatusNotFound)
			return
		}
...
func serveHTML(c *gin.Context, root fs.FS, name string) {
